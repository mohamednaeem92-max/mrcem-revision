...
if (plan.length !== 10 || artifact.applied?.length !== 10 || artifact.restricted?.length !== 0 || artifact.automaticApproval !== false) throw new Error("Batch 59 artifact count/status contract failed");
for (const item of plan) {
  ...
  if (draft.status !== "ocr_draft" || draft.askable !== true || draft.needsImage !== (item.needsImage ?? false) || draft.approved !== undefined) throw new Error(`OCR/mock safeguard failed: ${item.id}`);
...
console.log(JSON.stringify({ batch: 59, verified: 10, restored: 10, restricted: 0, automaticApproval: false, mockEligible: false }, null, 2));
