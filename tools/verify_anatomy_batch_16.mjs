...
const artifactPath = path.join(root, "docs", "audit", "applied-anatomy-batch-16.json");
const reviewedWarning = "Source page and external anatomy reference reviewed; remains an unapproved OCR draft.";
const restored = new Map([
  ["anatomy-anatomy-all-pdf-p0556-q0156", { page: 556, key: 4 }],
  ["anatomy-anatomy-all-pdf-p0558-q0158", { page: 558, key: 2 }],
  ["anatomy-anatomy-all-pdf-p0561-q0159", { page: 561, key: 1 }],
  ["anatomy-anatomy-all-pdf-p0563-q0160", { page: 563, key: 3 }],
  ["anatomy-anatomy-all-pdf-p0566-q0161", { page: 566, key: 2 }],
  ["anatomy-anatomy-all-pdf-p0567-q0162", { page: 567, key: 1 }],
  ["anatomy-anatomy-all-pdf-p0571-q0163", { page: 571, key: 1 }],
  ["anatomy-anatomy-all-pdf-p0572-q0164", { page: 572, key: 4 }],
  ["anatomy-anatomy-all-pdf-p0573-q0165", { page: 573, key: 4 }]
]);
const restricted = new Map([["anatomy-anatomy-all-pdf-p0557-q0157", { page: 557 }]]);
...
for (const item of artifact.restricted) {
  ...
  if (draft.sourcePage !== expectation.page || item.sourcePage !== expectation.page) throw new Error(`${item.id} has source-page drift.`);
  if (draft.status !== "needs_review" || draft.askable || draft.correctOption !== null || draft.approved === true) throw new Error(`${item.id} is not safely excluded.`);
