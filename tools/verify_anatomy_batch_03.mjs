/** Verifies post-application safeguards for Anatomy Batch 03. */
...
const artifactPath = path.join(root, "docs", "audit", "applied-anatomy-batch-03.json");
...
for (const item of artifact.applied) {
  ...
  if (!draft.highYieldNote?.trim() || !draft.mnemonic?.trim() || !draft.memoryAid?.coreFact?.trim()) throw new Error(`${item.id} lacks a source-linked learning aid.`);
