...
const snapshotPath = path.join(root, "docs", "audit", "anatomy-all-pdf-batch-73-ocr-records.json");
...
if (plan.length !== 10 || snapshotPayload.batch?.id !== "anatomy-all-pdf-batch-73") throw new Error("Batch 73 artifact contract failed");
...
for (const item of plan) {
  ...
  if (!draft || !snapshot) throw new Error(`Missing Batch 73 record: ${item.id}`);
...
await writeFile(artifactPath, `${JSON.stringify({ generatedAt: new Date().toISOString(), batch: "anatomy-all-pdf-batch-73", applied, restricted, automaticApproval: false, mockEligibility: "No restored OCR draft is mock eligible." }, null, 2)}\n`);
console.log(JSON.stringify({ batch: 73, restored: applied.length, restricted: restricted.length, automaticApproval: false, mockEligibility: "No restored OCR draft is mock eligible." }, null, 2));
