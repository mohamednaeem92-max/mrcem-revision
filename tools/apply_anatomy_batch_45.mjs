...
const snapshotPath = path.join(root, "docs", "audit", "anatomy-all-pdf-batch-45-ocr-records.json");
const planPath = path.join(root, "docs", "audit", "anatomy-batch-45-restorations.json");
const artifactPath = path.join(root, "docs", "audit", "applied-anatomy-batch-45.json");
...
if (plan.length !== 10 || snapshotPayload.batch?.id !== "anatomy-all-pdf-batch-45") throw new Error("Batch 45 artifact contract failed");

const applied = [];
for (const item of plan) {
  const draft = live.get(item.id);
  const snapshot = snapshots.get(item.id);
  if (!draft || !snapshot) throw new Error(`Missing Batch 45 record: ${item.id}`);
...
await writeFile(artifactPath, `${JSON.stringify({ generatedAt: new Date().toISOString(), batch: "anatomy-all-pdf-batch-45", applied, restricted: [], automaticApproval: false, mockEligibility: "No restored OCR draft is mock eligible." }, null, 2)}\n`);
console.log(JSON.stringify({ batch: 45, restored: applied.length, restricted: 0, automaticApproval: false, mockEligibility: "No restored OCR draft is mock eligible." }, null, 2));
