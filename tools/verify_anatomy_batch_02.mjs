/** Verifies post-application safeguards for Anatomy Batch 02. */
...
const artifactPath = path.join(root, "docs", "audit", "applied-anatomy-batch-02.json");
...
if (artifact.applied.length !== 7) throw new Error(`Expected seven restored records, found ${artifact.applied.length}.`);
if (artifact.restricted.length !== 3) throw new Error(`Expected three restricted records, found ${artifact.restricted.length}.`);
...
for (const item of artifact.applied) {
  ...
  if (!draft.highYieldNote?.trim() || !draft.mnemonic?.trim() || !draft.memoryAid?.coreFact?.trim()) throw new Error(`${item.id} lacks a complete source-linked learning aid.`);
