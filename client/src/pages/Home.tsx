...
import { loadOcrDraftQuestions, ocrDraftReport, ocrSectionManifest, type OcrDraftQuestion } from "@/lib/ocrDraftSections";
...
function mergeQuestionBanks(imported: Question[], extra: Question[] = []) {
  ...
  ocrQuestions.forEach((question) => merged.set(question.id, question));
  extra.forEach((question) => merged.set(question.id, question));
...
export default function Home() {
  ...
  const basePracticeBank = incorrectReviewSession ?? scheduledSession ?? questionBank;
  const activePracticeBank = subjectFilter ? basePracticeBank.filter((q) => q.subject === subjectFilter) : basePracticeBank;
  const currentQuestion = activePracticeBank[questionIndex] ?? activePracticeBank[0];
  const currentStatus = store.questions[currentQuestion.id];
  const attemptedCount = questionBank.filter((question) => (store.questions[question.id]?.attempts ?? 0) > 0).length;
  const totalAttempts = questionBank.reduce((sum, question) => sum + (store.questions[question.id]?.attempts ?? 0), 0);
  const correctAttempts = questionBank.reduce((sum, question) => sum + (store.questions[question.id]?.correct ?? 0), 0);
  ...
  const dailyQueue = useMemo(() => buildDailyQueue(questionBank.map((question) => question.id), store.questions), [questionBank, store.questions]);
  ...
  const reviewQuestions = useMemo(() => {
    return questionBank.filter((question) => {
      ...
      if (reviewFilter === "missed") return Boolean(record && record.attempts > 0 && record.correct < record.attempts);
      if (reviewFilter === "bookmarked") return Boolean(record?.bookmarked);
      if (reviewFilter === "unanswered") return !record?.attempts;
      return true;
    });
  }, [questionBank, reviewFilter, store.questions]);
  ...
  function openSourceLibrary(subject?: string) {
    setNotice(subject ? `${subject} remains in OCR review and is not eligible for active revision. Import approved JSON after source review.` : "OCR records remain restricted until source review is complete.");
    switchView("library");
  }
  ...
  function startDailyReview() {
    ...
    if (!queue.length) {
      ...
      setNotice("No approved questions are due today. Import reviewed questions or return when the next interval is due.");
      return;
    }
    setIncorrectReviewSession(null);
  ...
  function startIncorrectAnswerReview() {
    const queue = questionBank.filter((question) => {
      const record = store.questions[question.id];
      return Boolean(record && record.attempts > 0 && record.correct < record.attempts);
    });
    if (!queue.length) {
      setNotice("No previously incorrect approved questions are available yet. Continue revision and missed items will appear here.");
      return;
    }
    setScheduledSession(null);
    setMemoryRecallSession(null);
  ...
  const content =
    activeView === "dashboard" ? (
      ...
      <ReviewView
        ...
        incorrectCount={questionBank.filter((question) => { const record = store.questions[question.id]; return Boolean(record && record.attempts > 0 && record.correct < record.attempts); }).length}
        onStartIncorrectReview={startIncorrectAnswerReview}
      />
    ) : activeView === "library" ? (
      <DraftLibraryView verifiedCount={questionBank.length} onPractice={startVerifiedRevision} onImportBank={importReviewedQuestionBank} />
    ) : (
      <SettingsView
    ...
    <main className="min-h-screen bg-[#f6f5f0] text-[#183432]">
      ...
      <div className="relative flex min-h-screen">
        ...
        <section className="min-w-0 flex-1">
          <TopBar activeView={activeView} onMenu={() => setMenuOpen(true)} onNavigate={switchView} onExport={exportBackup} />
...
function TopBar({ activeView, onMenu, onNavigate, onExport }: { activeView: View; onMenu: () => void; onNavigate: (view: View) => void; onExport: () => void }) {
...
function DashboardView({ attemptedCount, overallAccuracy, bookmarks, activityDays, dueReviewCount, newReviewCount, store, questionBank, onPractice, onOpenQuestion, onOpenDraftSubject, onStartDailyReview, memoryAidCount, memoryRecallDueCount, memoryRecallNewCount, onStartMemoryRecall, ocrLoadStatus, ocrLoadCount, ocrLoadError, onRetryOcrLoad }: { attemptedCount: number; overallAccuracy: number; bookmarks: number; activityDays: number; dueReviewCount: number; newReviewCount: number; store: StudyStore; questionBank: Question[]; onPractice: () => void; onOpenQuestion: (question: Question) => void; onOpenDraftSubject: (subject: string) => void; onStartDailyReview: () => void; memoryAidCount: number; memoryRecallDueCount: number; memoryRecallNewCount: number; onStartMemoryRecall: () => void; ocrLoadStatus: "loading" | "loaded" | "error"; ocrLoadCount: number; ocrLoadError: string | null; onRetryOcrLoad: () => void }) {
  const nextQuestion = questionBank.find((question) => !store.questions[question.id]?.attempts) ?? questionBank[0];
  return (
      ...
      <section className="relative overflow-hidden rounded-[22px] border border-[#cfdcd6] bg-white shadow-[0_10px_28px_rgba(22,54,49,0.045)]">
        ...
        <div className="relative grid lg:grid-cols-[minmax(0,1fr)_320px]">
          <div className="p-6 sm:p-8">
              <div className="flex items-center gap-2"><span className="inline-flex items-center gap-1.5 rounded-md bg-[#e6f4ef] px-2 py-1 text-[10px] font-bold uppercase tracking-[0.13em] text-[#087c77]"><span className="h-1.5 w-1.5 rounded-full bg-[#087c77]" /> Verified study path</span>{ocrLoadStatus === "loading" ? <span className="text-[11px] font-semibold text-[#70847e]">Loading questions...</span> : ocrLoadStatus === "error" ? <span className="text-[11px] font-semibold text-red-600">Failed to load questions</span> : <span className="text-[11px] font-semibold text-[#70847e]">{questionBank.length} eligible · {ocrLoadCount} OCR loaded</span>}</div>
            <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="section-kicker">Next question</p><h2 className="mt-1 max-w-2xl font-serif text-[28px] leading-[1.15] tracking-[-0.028em] text-[#193d38] sm:text-[33px]">{nextQuestion.stem}</h2></div><div className="shrink-0 border-l-2 border-[#31534e] pl-3"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#6a817a]">Source trace</p><p className="mt-1 text-sm font-bold text-[#345750]">{nextQuestion.source} · p.{nextQuestion.sourcePage}</p></div></div>
            <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-[#dfe8e3] pt-5"><Button onClick={onPractice} className="h-10 bg-[#087c77] px-5 text-sm text-white shadow-[0_8px_20px_rgba(0,124,119,0.18)] hover:bg-[#066d69]">Resume revision <ArrowRight size={16} /></Button><button onClick={() => onOpenQuestion(nextQuestion)} className="flex h-10 items-center gap-2 rounded-lg border border-[#bfd1ca] bg-white px-4 text-sm font-semibold text-[#31554e] hover:bg-[#eff6f2]">Open item <ChevronRight size={16} /></button><span className="ml-auto hidden text-[11px] font-semibold text-[#668079] sm:inline">Answer first · explanation follows</span></div>
          </div>
          <div className="relative overflow-hidden border-t border-[#dfe8e3] bg-[#e9f0ec] p-6 lg:border-l lg:border-t-0">
            ...
            <div className="relative"><div className="flex items-center justify-between"><p className="section-kicker">Desk status</p><span className="rounded-md bg-[#103f3c] px-2 py-1 text-[9px] font-bold tracking-[0.14em] text-[#c4e4dc]">OFFLINE</span></div><div className="mt-6 space-y-4"><CommandMetric label="Verified items" value={`${questionBank.length}`} note={questionBank.length > pilotQuestions.length ? "Full bank loaded" : ocrLoadStatus === "loading" ? "Loading OCR bank..." : ocrLoadStatus === "error" ? "OCR load failed" : "EBM pilot loaded"} />{ocrLoadStatus === "loaded" ? <CommandMetric label="OCR loaded" value={`${ocrLoadCount}`} note="Questions from source PDFs" /> : ocrLoadStatus === "error" ? <div><CommandMetric label="OCR failed" value="--" note={ocrLoadError?.slice(0, 40) ?? "Unknown error"} /><button onClick={onRetryOcrLoad} className="mt-2 w-full rounded-lg bg-[#087c77] px-3 py-2 text-xs font-bold text-white hover:bg-[#066d69]">Retry load</button></div> : <CommandMetric label="OCR loading" value="..." note="Fetching 6.7 MB question bank" />}<CommandMetric label="Local backup" value="Ready" note={bookmarks ? `${bookmarks} saved bookmark${bookmarks === 1 ? "" : "s"}` : "Export from header"} /></div></div>
          </div>
        </div>
      </section>

      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard recordNo="01" label="Questions attempted" value={`${attemptedCount}/${questionBank.length}`} detail={ocrLoadStatus === "loaded" ? "Full bank loaded" : ocrLoadStatus === "loading" ? "Loading..." : "Pilot only"} trace="LOCAL · STUDY LOG" icon={BookOpenCheck} tone="teal" />
      ...
      <section className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_330px]">
        <div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {availableSubjects.map((subject) => {
              ...
              const draftSections = ocrSectionManifest.filter((section) => section.subject === subject);
              const draftTotal = draftSections.reduce((sum, section) => sum + section.questionCount, 0);
              const draftAskable = draftSections.reduce((sum, section) => sum + section.askableCount, 0);
              const meta = subjectMeta[subject];
              const subjectAttempts = questions.reduce((sum, question) => sum + (store.questions[question.id]?.attempts ?? 0), 0);
              const subjectCorrect = questions.reduce((sum, question) => sum + (store.questions[question.id]?.correct ?? 0), 0);
              const subjectAccuracy = subjectAttempts ? Math.round((subjectCorrect / subjectAttempts) * 100) : 0;
              const hasContent = Boolean(questions.length || draftTotal);
              return (
                <button
                  key={subject}
                  disabled={!hasContent}
                  onClick={() => questions.length ? onPractice() : onOpenDraftSubject(subject)}
                  className={cn(
                    "group relative overflow-hidden rounded-lg border p-5 pt-7 text-left transition",
                    hasContent ? "border-[#d1ddd7] bg-white shadow-[0_8px_22px_rgba(22,54,49,0.045)] hover:-translate-y-0.5 hover:border-[#759c91] hover:shadow-[0_14px_30px_rgba(22,54,49,0.08)]" : "cursor-not-allowed border-[#dfe4df] bg-[#eff1ed]/80 opacity-75",
                  )}
                >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2"><span className={cn("rounded-sm border px-2 py-1 font-sans text-[10px] font-bold tracking-[0.12em]", meta.tone)}>{meta.code}</span>{questions.length ? <img src="./manus-storage/meridian-mark_f22f4c75.png" alt="Meridian verified provenance seal" className="h-6 w-6 border border-[#79b9a9] bg-[#e7f3ee] p-0.5" /> : <span className="grid h-6 w-6 place-items-center border border-[#d7c39e] bg-[#fff7e8] text-[#9a6d28]"><FileText size={13} /></span>}</div>
                    {questions.length ? <span className="text-[11px] font-semibold text-[#31534e]">{questions.length} verified{draftTotal ? ` · ${draftTotal} OCR` : ""}</span> : draftTotal ? <span className="text-[11px] font-semibold text-[#a36d20]">{draftTotal} OCR drafts</span> : <span className="text-[11px] font-semibold text-[#83928c]">Awaiting import</span>}
                  </div>
                  <h3 className="mt-5 font-sans text-[19px] font-semibold tracking-[-0.02em] text-[#244642]">{subject}</h3>
                  <p className="mt-1.5 min-h-10 text-xs leading-5 text-[#71847e]">{meta.description}</p>
                  {questions.length ? (
                    <div className="mt-4 border-t border-[#e3e9e5] pt-3">
                      <div className="flex items-center justify-between text-[11px] text-[#607770]"><span>{subjectAttempts ? `${subjectAttempts} attempt${subjectAttempts === 1 ? "" : "s"}` : "Not started"}</span><span className="font-semibold text-[#345550]">{subjectAttempts ? `${subjectAccuracy}% accuracy` : "Ready"}</span></div>
                      <div className="mt-2 flex items-center gap-2"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[#e6efeb]"><div className="h-full rounded-full bg-[#087c77] transition-all duration-300" style={{ width: `${subjectAttempts ? subjectAccuracy : 0}%` }} /></div><span className="font-mono text-[9px] text-[#69827a]">{subjectAttempts ? `${subjectAccuracy}/100` : "00/100"}</span></div>
                      <div className="mt-3 flex items-center justify-between border-t border-dashed border-[#cbd9d2] pt-2 text-[9px] font-bold uppercase tracking-[0.12em] text-[#607971]"><span>Source trace</span><span className="flex items-center gap-1 text-[#31534e]"><img src="./manus-storage/meridian-mark_f22f4c75.png" alt="" className="h-3.5 w-3.5" /> Local verified record</span></div>
                    </div>
                  ) : draftTotal ? <div className="mt-4 grid grid-cols-2 gap-x-3 gap-y-3 border-t border-[#e1e5e1] pt-3"><div><p className="text-[9px] font-bold uppercase tracking-[0.13em] text-[#6d817a]">Detected keys</p><p className="mt-1 font-sans text-sm font-semibold text-[#31534e]">{draftAskable}</p></div><div><p className="text-[9px] font-bold uppercase tracking-[0.13em] text-[#6d817a]">Source state</p><p className="mt-1 font-sans text-sm font-semibold text-[#a36d20]">OCR draft</p></div><p className="col-span-2 border-l-2 border-[#d8a26b] pl-2 text-[10px] leading-4 text-[#8a6730]">Detected keys remain restricted; confirm every answer against the Markdown source, then import only approved records.</p><div className="col-span-2 flex items-center justify-between border-t border-dashed border-[#e2d8c8] pt-2 text-[9px] font-bold uppercase tracking-[0.12em] text-[#8a6730]"><span>Source trace pending</span><span className="flex items-center gap-1"><FileText size={12} /> OCR review</span></div></div> : <p className="mt-4 border-t border-[#e1e5e1] pt-3 text-[11px] text-[#83928c]">OCR review required before use.</p>}
                  </div>
                </button>
              );
            })}
        ...
        <aside className="relative overflow-hidden rounded-lg border border-l-[3px] border-[#cbd9d2] border-l-[#31534e] bg-[#fbfcfa] p-5 shadow-[0_8px_22px_rgba(22,54,49,0.045)]"><div className="absolute inset-0 clinical-rules opacity-45" aria-hidden="true" /><div className="relative">
          ...
          <div className="mt-5 rounded-xl bg-[#f0f6f3] p-4">
            <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#64827a]">{nextQuestion.topic}</p>
            <p className="mt-2 font-serif text-[17px] leading-6 text-[#284b46]">{nextQuestion.stem}</p>
            <button onClick={() => onOpenQuestion(nextQuestion)} className="mt-4 flex items-center gap-1.5 text-xs font-bold text-[#087c77] hover:text-[#066d69]">Open question <ArrowRight size={14} /></button>
          </div>
...
function PracticeView({ question, questionBank, index, selectedOption, submitted, record, onSelect, onSubmit, onMove, onBookmark, incorrectReviewSession, scheduledSession, onRate, showLearningAids, onToggleLearningAids, showEmojiCues, memoryAidRevealed, onRevealMemoryAid, subjectFilter, onSubjectFilterChange, subjects }: { question: Question; questionBank: Question[]; index: number; selectedOption: number | null; submitted: boolean; record?: Attempt; onSelect: (value: number) => void; onSubmit: () => void; onMove: (direction: 1 | -1) => void; onBookmark: () => void; incorrectReviewSession: boolean; scheduledSession: boolean; onRate: (rating: RecallRating) => void; showLearningAids: boolean; onToggleLearningAids: () => void; showEmojiCues: boolean; memoryAidRevealed: boolean; onRevealMemoryAid: () => void; subjectFilter: string | null; onSubjectFilterChange: (subject: string | null) => void; subjects: string[] }) {
  const isCorrect = selectedOption === question.correctOption;
  const memoryAid = hasCompleteMemoryAid(question.memoryAid) ? question.memoryAid : undefined;
  const swipe = useSwipe(() => onMove(1), () => onMove(-1));
  return (
    <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_308px]">
      <section className="min-w-0">
        {incorrectReviewSession && <div className="mb-5 rounded-xl border border-[#e9b9a7] bg-[#fff4f0] px-4 py-3 text-sm leading-5 text-[#854531]"><p className="font-bold">Incorrect-answer review</p><p className="mt-1 text-xs">These are approved questions you previously answered incorrectly. Re-attempt first, then use the explanation and learning aids to consolidate the gap.</p></div>}
          ...
          <div className="p-5 sm:p-8">
            <div className="space-y-3">
              {question.options.map((option, optionIndex) => {
                const showCorrect = submitted && optionIndex === question.correctOption;
                const showIncorrect = submitted && optionIndex === selectedOption && optionIndex !== question.correctOption;
                return (
                  <button
                    key={option}
                    disabled={submitted}
            ...
            {!submitted ? (
              <div className="mt-7 flex items-center justify-between border-t border-[#e5ebe7] pt-5"><p className="text-xs text-[#7b9089]">Choose one answer before checking the explanation.</p><Button disabled={selectedOption === null} onClick={onSubmit} className="h-10 bg-[#087c77] px-5 text-sm text-white hover:bg-[#066d69] disabled:bg-[#b9c9c2]">Check answer <ChevronRight size={16} /></Button></div>
            ) : (
              <div className={cn("mt-7 border-t pt-6", isCorrect ? "border-[#b8e0cc]" : "border-[#f0c3b6]")}>
                <div className={cn("flex items-center gap-3 rounded-xl px-4 py-3", isCorrect ? "bg-[#ebf8f0] text-[#236b4d]" : "bg-[#fff0ec] text-[#9a402b]")}><span className={cn("grid h-8 w-8 place-items-center rounded-full", isCorrect ? "bg-[#4aa77e] text-white" : "bg-[#d96b50] text-white")}>{isCorrect ? <Check size={17} /> : <X size={17} />}</span><div><p className="text-sm font-bold">{isCorrect ? "Correct" : "Not this time"}</p><p className="text-xs opacity-80">{isCorrect ? "Keep the principle, not only the answer." : question.correctOption !== null ? `Correct answer: ${String.fromCharCode(65 + question.correctOption)}.` : ""}</p></div></div>
                <div className="mt-5 grid gap-5 rounded-xl border border-[#d9e7e1] bg-[#f5faf7] p-5 sm:grid-cols-[minmax(0,1fr)_190px]">
                  <div><p className="section-kicker">Correct-answer explanation</p><p className="mt-2 flex items-start gap-2 text-sm font-bold leading-6 text-[#31554e]">{question.correctOption !== null && <><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[#087c77] text-[10px] text-white">{String.fromCharCode(65 + question.correctOption)}</span><span>{question.options[question.correctOption]}</span></>}</p><p className="mt-3 font-serif text-[17px] leading-7 text-[#2d4d48]">{question.explanation}</p></div>
                  <div className="border-t border-[#d8e6e0] pt-4 sm:border-l sm:border-t-0 sm:pl-5 sm:pt-0"><p className="section-kicker">Learning note</p><p className="mt-2 text-sm leading-6 text-[#486760]">{question.learningNote}</p><div className="mt-4 flex flex-wrap gap-1.5">{question.tags.map((tag) => <span key={tag} className="rounded-md bg-white px-2 py-1 text-[10px] font-semibold text-[#52716a]">{tag}</span>)}</div></div>
                </div>
      ...
      <aside className="space-y-4">
        <section className="rounded-2xl border border-[#d8e2dc] bg-white p-5 shadow-[0_8px_22px_rgba(22,54,49,0.04)]"><p className="section-kicker">Session map</p><div className="mt-4 grid grid-cols-4 gap-2">{questionBank.slice(0, 20).map((item, itemIndex) => <button key={item.id} onClick={() => onMove(itemIndex - index === 0 ? 1 : itemIndex > index ? 1 : -1)} className={cn("grid aspect-square place-items-center rounded-lg text-xs font-bold", item.id === question.id ? "bg-[#087c77] text-white" : "bg-[#edf4f0] text-[#55736b] hover:bg-[#dceae4]")}>{itemIndex + 1}</button>)}</div><p className="mt-4 text-xs leading-5 text-[#70867f]">{questionBank.length > pilotQuestions.length ? `${questionBank.length} verified questions are available offline.` : "This verified pilot contains four EBM questions. Source imports appear here after approval."}</p></section>
...
function DraftLibraryView({ verifiedCount, onPractice, onImportBank }: { verifiedCount: number; onPractice: () => void; onImportBank: (event: ChangeEvent<HTMLInputElement>) => void }) {
  const subjectSummaries = availableSubjects.map((subject) => {
    const sections = ocrSectionManifest.filter((section) => section.subject === subject);
    return {
      subject,
      sections,
      total: sections.reduce((sum, section) => sum + section.questionCount, 0),
      askable: sections.reduce((sum, section) => sum + section.askableCount, 0),
      review: sections.reduce((sum, section) => sum + section.needsReviewCount, 0),
      visuals: sections.reduce((sum, section) => sum + section.needsImageCount, 0),
    };
  });
  return <div className="max-w-6xl"><section className="rounded-[25px] bg-[#103f3c] p-6 text-white sm:p-8"><div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_260px]"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#a6d6cc]">Markdown section bank</p><h2 className="mt-3 max-w-2xl font-serif text-[30px] leading-[1.08] tracking-[-0.03em]">Your OCR questions are now grouped into revision sections.</h2><p className="mt-4 max-w-2xl text-sm leading-6 text-[#c4ddd7]">The app keeps all OCR drafts separate from verified questions. Detected answer keys are catalogue metadata only and cannot enter active revision or mock exams until a reviewer approves the source record.</p></div><div className="rounded-2xl bg-white/8 p-5"><FileText size={20} className="text-[#7ed2c2]" /><p className="mt-4 font-serif text-2xl">{ocrDraftReport.questionCount}</p><p className="mt-1 text-xs leading-5 text-[#b8d4cd]">OCR records across {ocrDraftReport.sectionCount} revision sections.</p><p className="mt-3 text-xs font-semibold text-[#7ed2c2]">{ocrDraftReport.askableCount} have a detected key, but none are active until approved.</p><div className="mt-4 border-t border-white/12 pt-4"><p className="text-[9px] font-bold uppercase tracking-[0.14em] text-[#a6d6cc]">Active verified bank</p><p className="mt-1 font-serif text-2xl text-white">{verifiedCount}</p><p className="mt-1 text-xs text-[#b8d4cd]">Eligible for revision and mocks.</p></div></div></div></section><section className="mt-7 space-y-4">{subjectSummaries.map(({ subject, sections, total, askable, review, visuals }) => { const meta = subjectMeta[subject]; return <article key={subject} className="overflow-hidden rounded-2xl border border-[#d8e2dc] bg-white shadow-[0_8px_22px_rgba(22,54,49,0.04)]"><div className="flex flex-col gap-4 border-b border-[#e3ebe6] px-5 py-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><span className={cn("rounded-lg border px-2 py-1 font-sans text-[10px] font-bold tracking-[0.12em]", meta.tone)}>{meta.code}</span><p className="font-serif text-xl text-[#294a45]">{subject}</p></div><p className="mt-2 text-xs text-[#71847e]">{total} OCR drafts · {askable} answer-key detected · {review} need text review · {visuals} may need an image</p></div><span className="rounded-lg border border-[#e4cdaa] bg-[#fff8e8] px-3 py-2 text-[11px] font-bold text-[#906b2b]">Restricted until approved</span>
</div><div className="grid gap-2 p-4 sm:grid-cols-2 lg:grid-cols-3">{sections.map((section) => <div key={section.path} className="rounded-xl border border-[#e0e8e3] bg-[#fbfcfb] p-3"><p className="min-h-10 text-sm font-semibold leading-5 text-[#31544e]">{section.topic.replace(`${subject} · `, "")}</p><p className="mt-2 text-[11px] text-[#6d817a]">{section.questionCount} total · {section.askableCount} key detected</p><p className="mt-3 text-[10px] font-bold uppercase tracking-[0.1em] text-[#9a7537]">Source review required</p>
</div>)}</div></article>; })}</section><section className="mt-6 flex flex-col gap-4 rounded-2xl border border-dashed border-[#bdcec7] bg-[#f2f7f4] px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm leading-6 text-[#517069]">Import a reviewed JSON bank when you have corrected an OCR section. Reviewed records remain separate from the OCR draft catalogue and are stored locally in this browser.</p><p className="mt-1 text-xs text-[#6e827b]">The original Markdown archive remains the source trace for every draft.</p></div><div className="flex flex-wrap gap-2"><label className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-lg bg-[#087c77] px-4 text-sm font-semibold text-white hover:bg-[#066d69]"><Upload size={15} /> Import reviewed JSON<input type="file" accept="application/json,.json" onChange={onImportBank} className="sr-only" /></label><Button onClick={onPractice} variant="outline" className="border-[#cbdad4] bg-white text-[#31544e] hover:bg-[#eff6f2]">Open verified bank <ArrowRight size={16} /></Button></div></section></div>;
}

function SettingsView({ onExport, onImport, onReset, hideLearningAidsByDefault, showEmojiCues, cueFirstMode, includeMemoryRecallWithDaily, onToggleLearningAidDefault, onToggleEmojiCues, onToggleCueFirstMode, onToggleIncludeMemoryRecallWithDaily }: { onExport: () => void; onImport: (event: ChangeEvent<HTMLInputElement>) => void; onReset: () => void; hideLearningAidsByDefault: boolean; showEmojiCues: boolean; cueFirstMode: boolean; includeMemoryRecallWithDaily: boolean; onToggleLearningAidDefault: () => void; onToggleEmojiCues: () => void; onToggleCueFirstMode: () => void; onToggleIncludeMemoryRecallWithDaily: () => void }) {
